...
def store_nbra(filename, ref_names, test_names, ref_prefix, test_prefix, param_set):
    ...
    with h5py.File(f'{filename}.hdf5', 'w') as f:
        for i in range(len(ref_names)):
            ...
            coupling_modifed = comparison_functions.model_parameter_coupling_modified(params)
            ...
            f.create_dataset(f'{i}/coupling_modifed', data = [coupling_modifed])
        ...
        for initial_state in initial_states:
            for coord in coords:
                for traj in trajs:
                    ...
                    name = f'../../data/{template}_{model_type}_{initial_state}_coord_{coord}_ntraj_{traj}'
                    name_mixed =  f'../../data/{template}_{model_type}_mixed_{initial_state}_coord_{coord}_ntraj_{traj}'
                    all_names_fssh = []
                    all_names_nbra_mixed = []
                    for j in sets:
                        all_names_fssh.append(f'{template}_{model_type}_{initial_state}_coord_{coord}_set_{j}_ntraj_{traj}_number_1')
                        all_names_nbra_mixed.append(f'{template}_{model_type}_mixed_{initial_state}_coord_{coord}_set_{j}_ntraj_{traj}_number_1')
