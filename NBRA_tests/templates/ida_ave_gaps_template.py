...
common_params.update({"rep_tdse":1, "rep_ham":0, "rep_sh":1, "rep_lz":0,
            ...
            "tsh_method":0, "decoherence_algo":1,
            "decoherence_times_type":1,  "dephasing_informed":1,
            ...
            for coord in initial_coords:
                for traj in trajs:
                    for number in numbers:
                        ...
                        name = f'ida_ave_gaps_{model_type}_{istate}_coord_{coord}_set_{i}_ntraj_{traj}_number_{number}'
                        name_adi_mixed = f'{model_type}_{mixed_istate}_coord_{coord}_set_{i}_ntraj_300_number_{number}'
                        name_mixed = f'ida_ave_gaps_{model_type}_mixed_{istate}_coord_{coord}_set_{i}_ntraj_{traj}_number_{number}'

                        if mixed:
                            AG = MATRIX(2,2)
                            AG_element = model_parameter_energy_gap({"ref_prefix": f"../../out/adiabatic_md_{name_adi_mixed}"})
                            AG.set(0, 1, AG_element)
                            AG.set(1, 0, AG_element)
                            ...
                            params_nbra_fssh.update({ "rep_ham":1, "force_method":0,
                                                     ...
                                                     "momenta_rescaling_algo":0, "x0":[coord], "ave_gaps": AG} )
                            ...
                            AG = MATRIX(2,2)
                            AG_element = model_parameter_energy_gap({"ref_prefix": f"../../out/adiabatic_md_{name_adi}"})
                            AG.set(0, 1, AG_element)
                            AG.set(1, 0, AG_element)
                            ...
                            AG_element = model_parameter_energy_gap({"ref_prefix": f"../../out/fssh_{name_adi}"})
                            AG.set(0, 1, AG_element)
                            AG.set(1, 0, AG_element)

                            params_fssh.update({"rep_ham":0, "force_method":1,  "nac_update_method":1,
                                        "tsh_method":0, "hop_acceptance_algo":20, "x0":[coord],
                                        "momenta_rescaling_algo":201, "ave_gaps": AG})
