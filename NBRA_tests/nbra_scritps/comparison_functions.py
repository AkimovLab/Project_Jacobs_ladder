...
def population_metric(params):
    ...
    ref_data_loc = params["ref_data_loc"]
    test_data_loc = params["test_data_loc"]

    with h5py.File(F"{ref_prefix}/mem_data.hdf", 'r') as f:
        reference_pop = np.array(f[ref_data_loc])
    with h5py.File(F"{test_prefix}/mem_data.hdf", 'r') as f:
        comparison_pop = np.array(f[test_data_loc])
    ...
    for timestep in range(timesteps):
        sum_over_states = 0.0
        for i in range(nstates):
            ref = reference_pop[timestep][i][i]
            comp = comparison_pop[timestep][i][i]
    ...
    return metric_at_timesteps # returns list with metric at each timestep
            ...
            for i in range(nstates):
                for j in range(nstates):
                    if i < j:
                        ...
                        sum_over_states += (abs(comp - ref))**2
                        cnt += 1
            sum_over_states = sum_over_states / cnt
            sum_over_traj += sum_over_states
        sum_over_traj = math.sqrt(sum_over_traj / ntraj)
        metric_at_timesteps.append(sum_over_states)
    ...
    for timestep in range(timesteps):
        ...
        for traj in range(ntraj):
            ...
            for i in range(nstates):
                ...
                sum_over_states += (abs(comp - ref))**2
            sum_over_states = sum_over_states / nstates
            sum_over_traj += sum_over_states
        sum_over_traj = math.sqrt(sum_over_traj / ntraj)
        metric_at_timesteps.append(sum_over_states)
